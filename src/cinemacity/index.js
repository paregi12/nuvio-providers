...
async function getStreams(tmdbId, mediaType, season, episode) {
    ...
    try {
        ...
        // Collect all decoded atob strings from the page scripts
        ...
        if (playerjsPath) {
            try {
                const playerjsUrl = playerjsPath.startsWith('http') 
                    ? playerjsPath 
        ...
        // Fallback to old extraction if dynamic script fetching failed
        if (!fileData) {
...
function findSeason(folders, season) {
    ...
    // 1. Regex check supporting English (Season/S) and Russian (Сезон)
...
function findEpisode(folderList, episode) {
    ...
    // 1. Regex check supporting English (Episode/Ep/E) and Russian (Серия)
