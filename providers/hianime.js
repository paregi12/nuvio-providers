// =====================================================
// HIANIME WORKING SCRAPER FOR NUVIO v3.0
// Based on tested extractors from GitHub
// Sources: HiAnime.to with proper embed extraction
// =====================================================
...
async function getEpisodeServers(animeId, episodeId, rid) {
  try {
    const url = `${CONFIG.HIANIME_AJAX}/v2/episode/servers?episodeId=${animeId}?ep=${episodeId}`;
    log(`[${rid}] Getting servers:`, { animeId, episodeId });
...
async function extractFromEmbed(embedUrl, rid) {
  ...
  try {
    ...
    if (lower.includes('megacloud') || lower.includes('vidplay') || lower.includes('mcloud')) {
      ...
      if (encryptedMatch) {
        try {
          const decoded = atob(encryptedMatch[1]);
          ...
          log(`[${rid}] Decode failed:`, e.message);
        }
      }
...
function formatStreams(allSources, mediaTitle) {
  ...
  allSources.forEach(serverData => {
    ...
    (sources.streams || []).forEach(stream => {
      ...
      // Skip embed URLs
      if (stream.url.includes('/embed/') || stream.url.includes('/e/')) return;
...
async function getStreams(tmdbId, mediaType, season, episode) {
  ...
  try {
    ...
    // Get servers
    const servers = await getEpisodeServers(bestMatch.id, targetEpisode.episodeId, rid);
    if (servers.length === 0) {
...
/*
// Usage:
getStreams('1429', 'tv', 1, 1)  // Attack on Titan S01E01
  .then(streams => {
